...
export function buildPrompt(
  ...
  const system = [
    ...
    '  9c. QUALIFIER ENFORCEMENT RULE: If the task contains BOTH a page/entity (e.g. "applications", "requests") AND a qualifier word describing a subset (e.g. "sent", "approved", "pending", "rejected", "on hold", "conditional"), you MUST apply the qualifier filter BEFORE calling done. Being on the correct page is NOT enough — the qualifier must be applied via the FILTER DROPDOWN (use `select`) or SEARCH BOX (use `input`). Do NOT call done saying "Already on the page" if a qualifier has not been applied yet. Re-read the task: if it says "show me SENT applications", your job is NOT done until the status filter shows "Sent". If the task says "show applications" with NO qualifier, then being on the page is sufficient.',
    ...
    '  14b. EXPORT RULE: For requests like "export to excel", "download excel", "export PDF", or "export government followup", complete steps in order: (1) navigate to the requested page/section if needed, (2) look for a button whose label contains Export/Excel/PDF (or Arabic equivalents: تصدير/إكسل/PDF) — if it exists and is NOT (disabled), click it and immediately call `done` with a success message. Downloads are async and produce no visible page change — do NOT retry or keep checking. (3) If the export button is (disabled), wait 600ms via `wait` then try clicking it. If still disabled after one retry, call `done` explaining it is unavailable. NEVER click table column headers (sort buttons) while looking for export.',
    ...
    '  19b. STALE INDEX RULE: After any URL change (visible in history as "→ navigated to"), ALL element indexes from BEFORE that navigation are INVALID and must NOT be used again. Always pick indexes ONLY from the CURRENT ELEMENTS LIST shown in this observation. If the LLM thinks "element 12 was the export button before navigation", discard that knowledge and find the export button in the CURRENT list.',
    ...
    'Example: {"thought":"User asked sent applications; select Sent status","action":"select","args":{"index":7,"value":"Sent"}}',
    'Example: {"thought":"User asked to export followup to excel; click export button","action":"click","args":{"index":12}}',
  ].join('\n')
